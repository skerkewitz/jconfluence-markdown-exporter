...
class ProgressUiTest {
    ...
    void plain_mode_emits_no_ansi_escapes() {
        String text = capture(false, ui -> {
            ...
            ui.pageDone(1, 3, PageId.of(42), "My Page", ProgressUi.Outcome.EXPORTED, Duration.ofMillis(320));
            ui.summary(stats(2, 0, 0), Path.of("/tmp/out"), Duration.ofMillis(1500));
        });
    ...
    void ansi_mode_clears_status_before_subsequent_lines() {
        String text = capture(true, ui -> {
            ui.status("step 1");
            ui.pageDone(1, 1, PageId.of(1), "p", ProgressUi.Outcome.EXPORTED, Duration.ofMillis(100));
        });
    ...
    void per_page_outcome_glyphs_differ() {
        String exported = capture(true, ui -> ui.pageDone(1, 3, PageId.of(1), "ok", ProgressUi.Outcome.EXPORTED, Duration.ofMillis(100)));
        String skipped  = capture(true, ui -> ui.pageDone(2, 3, PageId.of(2), "sk", ProgressUi.Outcome.SKIPPED, null));
        String failed   = capture(true, ui -> ui.pageDone(3, 3, PageId.of(3), "ng", ProgressUi.Outcome.FAILED, null));
    ...
    void summary_panel_includes_all_buckets() {
        ...
        String text = capture(true, ui -> ui.summary(stats, Path.of("/tmp/out"), Duration.ofMillis(12_345)));
    ...
    void summary_panel_skipped_when_nothing_done() {
        ...
        String text = capture(true, ui -> ui.summary(empty, Path.of("/tmp/out"), Duration.ofMillis(100)));
    ...
    void summary_uses_complete_title_when_no_failures() {
        ...
        stats.incExported();
        String text = capture(true, ui -> ui.summary(stats, Path.of("/tmp/out"), Duration.ofMillis(100)));
        assertThat(text).contains("Export complete");
    ...
    void format_ms_handles_sub_second_seconds_and_minutes() {
        assertThat(ProgressUi.formatDuration(Duration.ofMillis(450))).isEqualTo("450 ms");
        assertThat(ProgressUi.formatDuration(Duration.ofMillis(1_500))).isEqualTo("1.5s");
        assertThat(ProgressUi.formatDuration(Duration.ofMillis(72_000))).isEqualTo("1m 12s");
    }
    ...
    void format_ms_uses_dot_decimal_separator_under_german_locale() {
        ...
        try {
            ...
            assertThat(ProgressUi.formatDuration(Duration.ofMillis(1_500))).isEqualTo("1.5s");
            assertThat(ProgressUi.formatDuration(Duration.ofMillis(12_345))).isEqualTo("12.3s");
        } finally {
    ...
    void silent_factory_writes_nothing() {
        ...
        silent.phase("x");
        silent.status("y");
        silent.pageDone(1, 1, PageId.of(1), "p", ProgressUi.Outcome.EXPORTED, Duration.ofMillis(100));
        silent.summary(new ExportStats(1), Path.of("/tmp"), Duration.ofMillis(100));
    }
